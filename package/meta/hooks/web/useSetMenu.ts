...
export function useSetMenu() {
  ...
  function setMenuListAndFirstMenu(routes: AppRouteRecordRaw[]) {
    const firstNode = filterFirstTreeNode(routes, (node) => {
   ...
   * @returns {any}
   */
  function generateMenuList(routeList: AppRouteRecordRaw[]): AppRouteRecordRaw[] {
    return routeList.map((route) => {
      ...
      if (route.children && route.children.length)
        menu.children = generateMenuList(route.children) as any
